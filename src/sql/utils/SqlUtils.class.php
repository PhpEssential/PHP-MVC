<?php
namespace phpessential\mvc\sql\utils;

use phpessential\mvc\sql\core\metadata\Field;
use phpessential\mvc\utils\DateUtils;
...
class SqlUtils {
	...
	public static function ensureSqlValue(Field $field, $value): string {
		...
		switch ($field->dataType) {
			case Field::DATE :
				return $value->format(DateUtils::SQL_DATE_FORMAT);
			case Field::DATE_TIME :
				return $value->format(DateUtils::SQL_DATE_TIME_FORMAT);
			case Field::BOOLEAN :
				return $value ? "1" : "0";
			case Field::FLOAT :
				return strval($value);
			case Field::INT :
				return strval($value);
			case Field::TEXT :
				return htmlspecialchars(str_replace("%", "\\%", str_replace("_", "\\_", str_replace("'", "\\'", $value))));
			default :
