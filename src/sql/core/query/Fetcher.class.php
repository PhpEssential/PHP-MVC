...
use phpessential\mvc\exception\SqlException;
use phpessential\mvc\sql\core\DbConnection;
use phpessential\mvc\sql\core\metadata\DirectAssociation;
use phpessential\mvc\sql\core\metadata\ExtraAssociation;
...
class Fetcher {
	 ...
	 *        	- Path to foreign entity field
	 * @param ConditionExpression $conditions
	 *        	- Additional conditions
	 * @param bool $onlyDiscriminants
	...
	public function fetch($path, bool $onlyDiscriminants = false, bool $strict = false): Fetcher {
		...
		if ($field instanceof Field) {
			...
			foreach ( $field->linkFields as $linkField ) {
				$foreignFields [] = $linkField->referenceField;
			}
			...
			$primaryFields = array ();
			foreach ( $field->linkFields as $linkField ) {
				$fieldName = $field->name . "-" . $linkField->referenceField->name;
				$primaryFields [] = new Field($fieldName, $linkField->sqlName, $linkField->referenceField->dataType);
			}
	 ...
	 *
	 * @param string $path
	 *        	- Path of field to fetch
	 * @param DirectAssociation $field
	 *        	- Field to fetch
	 ...
	 * @param bool $strict
	 *        	- If true JOIN is generate else it is a LEFT OUTER JOIN
	 * @param bool $onlyDiscriminantFields
	 *        	- If true that do not select all foreign entity's fields
	 */
		...
		foreach ( $field->linkFields as $referenceField ) {
			foreach ( $foreignFields as $foreignField ) {
				foreach ( $primaryFields as $primaryField ) {
					if ($foreignField->field->name == $referenceField->referenceField->name && $primaryField->field->sqlName == $referenceField->sqlName) {
	...
	private function processExtra(array &$entities, DbConnection $conn, string $dbName) {
		...
		foreach ( $this->extras as $flatPath => $fetcher ) {
			...
			if ($listField instanceof ManyToMany) {
				...
				$linkFields = array ();
				foreach ( $listField->linkFields as $linkField ) {
					$linkFields [] = $fetcher->select->findField($listField->referenceField->name . "-" . $linkField->referenceField->name);
				}
			} else {
	...
	private function getSqlValueForEntity(Field $field, string $sqlValue = null) {
		...
		switch ($field->dataType) {
			...
			case Field::TEXT :
				return $sqlValue;
			default :
				throw new \Exception("Type inconnu: " . $field->dataType);
		}
	}
	...
	/**
	 * Search the corresponding select path in secondaries select.
